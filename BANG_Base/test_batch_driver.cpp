...
double calculate_recall(unsigned batchSize, unsigned nIter, unsigned *gold_std,
						...
						unsigned recall_at)
{
	...
	for (size_t i = 0; i < batchSize; i++)
	{
		...
		gt.clear();
		res.clear();
		unsigned *gt_vec = gold_std + dim_gs * ((batchSize * nIter) + i);
		...
		if (gs_dist != nullptr)
		{
			...
			float *gt_dist_vec = gs_dist + dim_gs * ((batchSize * nIter) + i);
...
int run_anns(int argc, char **argv)
{
    ...
    if (batchSize > numQueries || numQueries % batchSize != 0) {
		printf("Error.. numQueries must be larger than batchSize and the mod is 0");
		return -1;
    }
	...
	int nWLLen = 200;
	int nIter = 0;
    int totalIter = numQueries / batchSize;
	...
	if (DISTFUNC_MIPS == argv[8])
		...
		do
		{
            if (nIter == 0)
            {
                cout << "Iter\t" << "Time \t" << "QPS\t" << "\t" << recall_param <<"-r@" << recall_param << endl;
                cout << "--\t" << "---- \t" << "---\t" << "\t------" << endl;
            }
                
			objBANG.bang_set_searchparams(recall_param, nWLLen, uDistFunc);
			objBANG.bang_alloc(batchSize);

			for (int nCounter = 0; nCounter < 1; nCounter++)
			{
				result_ann_t *nearestNeighbours = (result_ann_t *)malloc(sizeof(result_ann_t) * recall_param * batchSize);
				float *nearestNeighbours_dist = (float *)malloc(sizeof(float) * recall_param * batchSize);
				vector<vector<result_ann_t>> final_bestL1; // Per query vector to store the visited parent and its distance to query point
				final_bestL1.resize(batchSize);

				objBANG.bang_init(batchSize);
				auto milliStart = checkpoint_time_millisec ();

				objBANG.bang_query(queriesFP + Dim * batchSize * nIter, batchSize, nearestNeighbours, nearestNeighbours_dist);

				auto milliEnd = checkpoint_time_millisec ();
				double totalTime_wallclock = milliEnd - milliStart;
				double throughput = (batchSize * 1000.0) / totalTime_wallclock;
...
#pragma omp parallel
				{
					...
					for (unsigned ii = CPUthreadno; ii < batchSize; ii = ii + numCPUthreads)
					{
						query_best.clear();
...
#pragma omp critical
				...
				for (unsigned test_id = 0; test_id < Lvec.size(); test_id++)
				{
					...
					query_result_ids[test_id].resize(recall_param * batchSize);

					for (unsigned ii = 0; ii < batchSize; ++ii)
					{
					...
					if (calc_recall_flag)
						recall = calculate_recall(batchSize, nIter, gt_ids, gt_dists, gt_dim, 
						query_result_ids[test_id].data(), recall_param, recall_param);
					//cout << "Ls\t" << recall_string << endl;
					//cout << L1 << "\t" << recall << endl;
				}

				cout << nIter << "\t" << totalTime_wallclock << "\t" << throughput << "\t" <<  recall << endl;
...
int main(int argc, char **argv)
{
	...
	if (argc < 8)
	{
		...
		<query file> <GroundTruth File> <NumQueries> <batchSize> <recall parameter k> <data type : uint8, int8 or float> <dist funct: l2 or mips>"
			 << endl;
		exit(1);
	}
